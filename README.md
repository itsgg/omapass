...
- 󰐕 **Create, edit and archive**: logins, passwords, cards and secure notes, from a form built per category. An edit sends only the fields you changed and is previewed with `op --dry-run` before anything is written, so it cannot rewrite what it does not show. Archiving asks first and is recoverable from the app.
...
| ![Creating a login](docs/demo-create.png) | ![Editing an item](docs/demo-edit.png) |
| One form per category, built from a spec rather than hand-written. The password is generated by 1Password unless you say otherwise. | An edit names only the fields you changed, so nothing it does not show can be rewritten. |
| ![Archiving an item](docs/demo-archive.png) | |
| Removing asks first, archives rather than deletes, and starts on **Keep**. | |
