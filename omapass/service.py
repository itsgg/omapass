...
class OmaPassService:
    ...
    def create_login(
        ...
        """Creates a login with a password generated by 1Password itself.

        The password is never seen here. `--generate-password` makes `op`
        produce and store it, so no credential passes through this process,
        its arguments, or its memory. That is the whole reason this is the
        first write operation: it cannot leak a secret it never holds.

        Title, username and URL are not secrets, but they are still the user's
        data, so they travel in a 0600 template file rather than in argv where
        every process on the machine could read them.
        """
        if not self.check_op_installed():
            return {"ok": False, "error": "op CLI not installed"}
        ...
        if not password and not PASSWORD_RECIPE_RE.match(recipe):
        ...
        if password:
            # A password the user typed has to reach op somehow, and op takes
            # field values either as arguments or in a template. Arguments are
            # world-readable in /proc, so it goes in the template: mode 0600,
            # in $XDG_RUNTIME_DIR, which is tmpfs, and unlinked as soon as op
            # returns. A generated password avoids this entirely by never
            # existing outside 1Password, which is why it is the default.
        ...
        if normalized_url:
            template["urls"] = [
                {"label": "website", "primary": True, "href": normalized_url}
            ]
        ...
        try:
            ...
            # The template holds no password, but it is still the user's data
            # and has no reason to outlive the command.
            try:
        ...
        self._note_op_success(epoch)
        if not dry_run:
            # The new item has to appear in the list, and only a sync knows it.
            threading.Thread(target=self.sync, daemon=True).start()
